...
namespace TheEngine::RenderingSystem::OpenGLBackend
{
	...
	RenderTarget  GPUTextureManager::createRenderTarget(const RenderTargetCreateInfo& renderTargetCreateInfo)
	{
		...
		if (renderTargetCreateInfo.makeResident)
		{
			residentHandle = glGetTextureHandleARB(glTextureId);
	...
	//m_allotedGLTextureIDs.push_back(glTextureId); // For destruction tracking
		...
		RenderTargetInfo.apiHandle = static_cast<uint64_t>(glTextureId);
		RenderTargetInfo.residentHandle = residentHandle;
		RenderTargetInfo.type = renderTargetCreateInfo.type;
		RenderTargetInfo.width = renderTargetCreateInfo.width;
		RenderTargetInfo.height = renderTargetCreateInfo.height;
		RenderTargetInfo.internalFormat = renderTargetCreateInfo.internalFormat;
		//info.samplerSettings = samplerSettings;
