...
namespace TheEngine
{
	...
	*/
	...
	private:
		...
		std::unordered_map<MaterialType, Memory::GPUBufferSubBumpAllocator> m_materialTypeToSSBO;

		MaterialId m_currentMaterialId = 0;

	public:
		GPUMaterialSystem(Memory::GPUBufferManager& gpuBufferManager);
		MaterialId uploadMaterial(MaterialType materialType, std::byte* ptr, size_t size); //size in bytes

		const Memory::GPUBufferInfo getGPUBufferInfo(MaterialType materialType);
